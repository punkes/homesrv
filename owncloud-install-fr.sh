...
Bienvenue dans ce guide complet pour créer votre propre serveur à domicile en utilisant un NUC recyclé ou tout autre PC ayant une configuration similaire ou meilleure. Ce serveur polyvalent sera installé avec le système de virtualisation Proxmox. Il sera capable de faire : de l'hébergement web 🌐, un serveur VPN 🔒, et un serveur de fichiers disponible en ligne, cloud 📁.
...
Avant de commencer, assurez-vous de posséder des connaissances minimes en matière de matériel informatique, de virtualisation, de systèmes Linux Debian, ainsi que de réseaux.

## Matériel 💻

### Spécifications Matérielles du PC
...
### Clé Usb
Une clé USB (4 Go ou plus)

## Installation 🛠️

### Téléchargement de Proxmox

Pour pouvoir installer proxmox sur le nux, il est nécéssaire de télécharger l'iso de proxmox et le logiciel rufus pour graver cet iso sur une clé usb.
...
[OwnCloud](https://owncloud.com/) est un logiciel gratuit qui permet de créer son propre cloud gratuit.

**Étape 1 : Se connecter avec l'utilisateur root sur la VM Debian**

Si vous n'êtes pas encore connecté, connecter vous avec l'utilisateur root.
Sinon déconnecter vous avec la commande :
   ```bash
    exit
   ```


**Étape 2 : Récupérer le fichier'owncloud-install-fr.sh'**
   
   Installer curl pour pouvoir récuperer le fichier.
   ```bash
...
Récupérer le fichier
   ...
   ```bash
   curl https://raw.githubusercontent.com/TheGostOfNight/DIY_Home_Server/main/owncloud-install-fr.sh
   ```
